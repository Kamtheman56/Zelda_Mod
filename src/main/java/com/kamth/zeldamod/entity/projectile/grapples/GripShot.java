...
public class GripShot extends AbstractArrow {
    private static final double BASE_DAMAGE = 6.0D;
    private final ItemStack hookshot = new ItemStack(ZeldaItems.GRIPSHOT.get());
    ...
    public GripShot(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
    ...
    public GripShot(Level world, Player user) {
        super(ModEntityTypes.GRIPSHOT.get(), user, world);
        this.pickup = Pickup.DISALLOWED;
    }
    @Override
            ...
            if (!level().isClientSide) {
                ...
                if (owner != null) {
                    if (owner.isDeadOrDying() || this.tickCount == 50 ||
                            !(owner.getMainHandItem().getItem() instanceof GripshotItem ||
                                    owner.getOffhandItem().getItem() instanceof GripshotItem)){
                        kill();
                    }
